	...
	for(i = 0; i < summary.col.length; i++) {
		...
		for(j = 0; j < responses.length; j++) {
			...
			if (summary.col[i].datatype === 'check' || summary.col[i].datatype === 'checkmaybe') {
				...
				// console.log("Adding ", v, " to ", summary.col[i].summary);
				summary.col[i].summary += v;
				// console.log("Became ", summary.col[i].summary);
			} else {
				summary.col[i].summary++;
			}
	...
	for(i = 0; i < summary.col.length; i++) {
		...
		if (summary.col[i].restrictions && summary.col[i].restrictions.enabled) {
			if (summary.col[i].restrictions.maxcheck) {
				...
				summary.col[i].restrictions.isLocked = (summary.col[i].summary >= summary.col[i].restrictions.maxcheck);
			}
			if (summary.col[i].restrictions.maxnum) {
				summary.hasRestrictions = true;
				summary.col[i].restrictions.isLocked = (summary.col[i].summary >= summary.col[i].restrictions.maxnum);
				summary.col[i].restrictions.remaining = (summary.col[i].restrictions.maxnum - summary.col[i].summary);
			}
...
FoodleSchema.methods.allowedResponses = function(feideconnect) {

	if (this.publicresponses) {
		return true;
	}
...
FoodleSchema.methods.requireAllowedResponses = function(feideconnect) {

	if (!this.allowedResponses(feideconnect)) {
		throw new Error("You are now allowed to see responses to this foodle.");
	}
