...
var 
	...
	QueryEngine = require('./QueryEngine').QueryEngine,
...
API.prototype.init = function() {
	...
	mongoose.connect(this.dbstr, options);
}
...
API.prototype.setupRoute = function() {
	...
	.get(function(req, res) {
		...
		var engine = new QueryEngine(req.feideconnect.userid);
		engine.fetch()
	...
	this.router.route('/foodles/listing/')
	.post(function(req, res) {

		var engine = new QueryEngine(req.feideconnect.userid);
		engine.setGroups(req.body.groups);
		engine.fetch()
			.then(function(foodle) {
				res.json(foodle);
			}, function(err) {
